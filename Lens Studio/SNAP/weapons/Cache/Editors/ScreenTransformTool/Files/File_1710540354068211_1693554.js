...
let Gizmo = Gizmo_1 = class Gizmo extends BaseScriptComponent {
    ...
    setLayer(layer) {
        ...
        layer = layer.union(_GizmoUtils.module.GizmoUtils.GIZMO_LAYER);
    ...
    containsWorldPoint(point) {
        return this.containsScreenPoint(_GizmoUtils.module.GizmoUtils.worldPointToScreenPoint(this.gizmoScreenTransform, point));
    }
    ...
    processWorldDelta(worldPoint, touchType) {
        if (touchType === _SidePoint.module.TouchEventType.STARTED) {
    ...
    setupGizmoInteractions() {
        ...
        const onTouchStart = (eventData) => {
            ...
            if (camera) {
                screenPosition = _GizmoUtils.module.GizmoUtils.screenPointToScreenPoint(screenPosition, this.interactionCamera, camera);
            }
            Gizmo_1.isBusy = true;
            ...
            Gizmo_1.ActiveGizmos.forEach((gizmo) => gizmo.processWorldDelta(worldTouchPosition, _SidePoint.module.TouchEventType.STARTED));
        ...
        const onTouchMove = (eventData) => {
            ...
            Gizmo_1.ActiveGizmos.forEach(gizmo => gizmo.processWorldDelta(translatedTouchPosition(eventData), _SidePoint.module.TouchEventType.CHANGED));
        ...
        const onTouchEnd = (eventData) => {
            ...
            Gizmo_1.ActiveGizmos.forEach(gizmo => gizmo.processWorldDelta(translatedTouchPosition(eventData), _SidePoint.module.TouchEventType.ENDED));
            Gizmo_1.ActiveGizmos.forEach(gizmo => gizmo.convertOffsetToAnchors());
            this.align({ screenTransform: this.editableScreenTransform, interactionType: _AlignmentTypes.module.InteractionType.Move,
                extraScreenTransforms: Gizmo_1.ActiveGizmos.map(gizmo => gizmo.getEditableScreenTransform()) });
            Gizmo_1.ActiveGizmos.forEach(gizmo => gizmo.convertAnchorsToOffset());
    ...
    __initialize() {
        ...
        this.processRotationPointUpdate = (eventData) => {
            ...
            const roundedAngle = _GizmoUtils.module.GizmoUtils.roundAngleToDegrees(angleRad * 180 / Math.PI, _KeyboardListener.module.KeyboardListener.isKeyPressed(Keys.Key_Shift) ? 15 : 1) / 180 * Math.PI;
            ...
            if (eventData.touchEventType === _SidePoint.module.TouchEventType.CHANGED) {
                this.mouseHint.show();
            }
            else if (eventData.touchEventType === _SidePoint.module.TouchEventType.ENDED) {
                this.mouseHint.hide();
            }
        ...
        this.processSidePointUpdate = (eventData) => {
            ...
            if (eventData.touchEventType === _SidePoint.module.TouchEventType.STARTED) {
            ...
            if (Math.abs(eventData.sidePointOffset.x) > this.EPS
                ...
                && _KeyboardListener.module.KeyboardListener.isKeyPressed(Keys.Key_Shift)) {
