...
class AlignerHelper {
    constructor(tree, camera, lineRenderer, guides) {
        ...
        this.vertical = [];
        this.align = (data) => {
            if (!_Config.module.Config.isSnappingEnabled.value || _KeyboardListener.module.KeyboardListener.isKeyPressed(Keys.Key_Shift)) {
                return;
            }
    ...
    alignMove(screenTransforms) {
        ...
        screenTransforms.forEach(scr => _GizmoUtils.module.GizmoUtils.setPivotPreservePosition(scr, vec2.zero()));
        ...
        screenTransforms.forEach((scr, idx) => _GizmoUtils.module.GizmoUtils.setPivotPreservePosition(scr, savePivots[idx]));
    ...
    alignScale(screenTransform, direction) {
        ...
        if (Math.abs(direction.x) > this.EPS && Math.abs(direction.y) > this.EPS) { // CORNER
            if (Math.abs(deltas.y) < _Config.module.Config.MaxAttachScreenDistance) {
                cornerScreenPoint.x += deltas.y;
            }
            if (Math.abs(deltas.x) < _Config.module.Config.MaxAttachScreenDistance) {
            ...
            if (Math.abs(res.x) < _Config.module.Config.MaxAttachScreenDistance && Math.abs(res.y) < _Config.module.Config.MaxAttachScreenDistance) {
    ...
    collectAlignPoints() {
        ...
        this.screenTransforms.forEach((screenTransform) => {
            if (!rootIsAdded && !isNull(screenTransform.getSceneObject().hasParent())
                ...
                this.cornersLocalPoint.forEach((corner) => {
                    addPoint(_GizmoUtils.module.GizmoUtils.parentPointToScreenPoint(screenTransform, corner));
                });
                rootIsAdded = true;
            }
            this.cornersLocalPoint.forEach((corner) => {
